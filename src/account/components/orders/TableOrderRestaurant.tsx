    ...
    <div key={order._id} className="animate-fade-in">
      <div className="bg-zinc-800 bg-opacity-80 px-4 py-3 flex items-center justify-between border-b border-zinc-700 border-opacity-50">
        ...
        <div className="flex items-center gap-4 whitespace-nowrap">
          ...
          <div className="w-fit">
            ...
            >
              <img src={QRCodeGenerator(`${window.location.origin}/table-orders/${order.restaurant._id}/by/${order._id}`)} alt={`Código QR del restaurante ${order.restaurant.name}`} className="w-full h-full object-contain" />
      ...
      <div className="bg-zinc-800 bg-opacity-80 px-4 py-3 flex items-center gap-4 justify-between border-b border-zinc-700 border-opacity-50 overflow-auto whitespace-nowrap">
        ...
        <div className="flex items-center justify-end gap-2 animate-fade-in">
          ...
          <Link to={`/table-orders/${order.restaurant._id}/by/${order._id}`} className="text-center min-w-[100px] text-sm transition-colors duration-300 rounded-md px-2 py-1 font-semibold bg-pink-300 text-pink-400 border border-pink-400 bg-opacity-10 border-opacity-40 hover:bg-opacity-20">
            Visualizar
          </Link>
        </div>
      </div>
