...
import useChangeStatusTableOrder from '@/restaurant/hooks/useChangeStatusTableOrder';
...
interface OrderProps {
  order: TableOrderByRestaurant;
  updateOrder: (order: TableOrderByRestaurant) => void;
}

const TableOrder = ({ order, updateOrder }: OrderProps) => {
  const [statusSelected, setStatusSelected] = useState<TableOrderStatus | null>(order.status);
  const { loadingStatus, changeStatus } = useChangeStatusTableOrder();

  const handleStatus = async (e: React.FormEvent<HTMLSelectElement>) => {
    const statusSelected = e.currentTarget.value as TableOrderStatus;
