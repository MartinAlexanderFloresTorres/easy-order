...
import { TableOrderPayment } from '@/restaurant/interfaces';
...
import usePayTableOrder from '@/restaurant/hooks/usePayTableOrder';

let timer: NodeJS.Timeout | null = null;

const TableOrderPaymentPage = () => {
  const [loading, setLoading] = useState(true);
  const [order, setOrder] = useState<TableOrderPayment | null>(null);
  ...
  const { loadingPayOrder, payOrder } = usePayTableOrder();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await ClientAxios.get<TableOrderPayment>(`/table-order/${restaurantId}/by/${orderId}`, {
          headers: {
...
export default TableOrderPaymentPage;
