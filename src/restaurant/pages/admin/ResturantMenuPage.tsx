import MenuTable from '@/restaurant/components/admin/menu/MenuTable';
import ModalFormMenu from '@/restaurant/components/admin/menu/ModalFormMenu';
import useRestaurant from '@/restaurant/hooks/useRestaurant';
import { Menu } from '@/restaurant/interfaces';
import { useState } from 'react';

const RestaurantMenuPage = () => {
  const [isShowFormMenu, setIsShowFormMenu] = useState(false);
  const [menuEdit, setMenuEdit] = useState<Menu | null>(null);

  const { restaurant, menus, loadingMenus } = useRestaurant();

  if (!restaurant) return;

  const editMenu = (menu: Menu) => {
    setMenuEdit(menu);
    setIsShowFormMenu(true);
  };

  const closeModal = () => {
    setIsShowFormMenu(false);
    setMenuEdit(null);
  };
  return (
    <>
      <div className="select-none w-full backdrop-blur-sm animate-fade-in">
        <div className="border-t border-l border-r border-zinc-700 border-opacity-50 bg-zinc-800 bg-opacity-80 flex items-stretch justify-between text-center">
          ...
          <div className="px-4 py-3 flex items-center justify-center text-center flex-1">
            <h3 className="text-2xl font-semibold text-gray-200 uppercase text-center">Menús</h3>
          </div>
          <div className="px-4 py-3 flex items-center justify-end text-center gap-4 border-l border-l-zinc-700 border-opacity-50">
            <button
              ...
              onClick={() => setIsShowFormMenu(true)}
            >
              Crear menú
            </button>
          </div>
        ...
        <MenuTable menus={menus} loadingMenus={loadingMenus} editMenu={editMenu} />
      </div>

      {isShowFormMenu && <ModalFormMenu onClose={closeModal} menuEdit={menuEdit} />}
    </>
  );
};
