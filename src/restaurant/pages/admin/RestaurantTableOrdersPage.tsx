...
import { TableOrderByRestaurant } from '@/restaurant/interfaces';
import TableOrder from '@/restaurant/components/order/TableOrder';

const RestaurantTableOrdersPage = () => {
  const [orders, setOrders] = useState<TableOrderByRestaurant[]>([]);
  ...
  useEffect(() => {
    if (restaurant) {
      (async () => {
        try {
          const { data } = await ClientAxios.get<TableOrderByRestaurant[]>(`/table-order/${restaurant._id}`, {
            headers: {
  ...
  const updateOrder = (order: TableOrderByRestaurant) => {
      ...
      <div className="backdrop-blur-sm border-b border-b-zinc-700 border-opacity-50 bg-zinc-800 bg-opacity-80 flex items-stretch justify-between text-center">
        ...
        <div className="px-4 py-3 flex items-center justify-center text-center flex-1">
          <h3 className="text-2xl font-semibold text-gray-200 uppercase text-center">
            Ordenes <span className="text-pink-600">({orders.length})</span>
          </h3>
        </div>
      </div>
        ...
        {loadingOrder ? (
          <div className="px-4 py-20 bg-zinc-800 bg-opacity-50 text-center animate-fade-in">
            <h2 className="font-extrabold text-center uppercase text-gray-300 mb-4">
              Buscando tus ordenes <span className="text-pink-600">🔎</span>
            </h2>
            <p className="text-gray-400 text-center mb-6 max-w-lg mx-auto">Estamos buscando tu ordenes, por favor espera un momento.</p>
            <Spinner className="mx-auto text-gray-400" />
          </div>
        ) : (
          <>
            {orders.length ? (
              orders.map((order) => <TableOrder key={order._id} order={order} updateOrder={updateOrder} />)
            ) : (
              <div className="px-4 py-20 bg-zinc-800 bg-opacity-50 text-center animate-fade-in">
                <h2 className="font-extrabold text-center uppercase text-gray-300 mb-4">
                  No han realizado ningúna orden <span className="text-pink-600">😢</span>
                </h2>
                <p className="text-gray-400 text-center mb-4 max-w-lg mx-auto">
                  Espera a que un cliente realice una orden para que puedas verlo aquí.🍔
                </p>
...
export default RestaurantTableOrdersPage;
